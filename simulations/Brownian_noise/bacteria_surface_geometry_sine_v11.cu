...
//bacteria surface geometry, sine boundary, Brownian noise
...
__device__ double d2y_dx2_sin_curve(double3 S, double A, double lambda)
{
  double TwoPiLambda = ((2.0 * PI) / lambda);
  double d2y_dx2;

  if (S.y < 0)
  {
    d2y_dx2 = A * TwoPiLambda * TwoPiLambda * sinpi((2.0 * S.x) / lambda);
  }
  else
  {
    d2y_dx2 = -A * TwoPiLambda * TwoPiLambda * sinpi((2.0 * S.x) / lambda);
...
__device__ double dydx_sin_curve(double3 S, double A, double lambda)
{
  double TwoPiLambda = ((2.0 * PI) / lambda);
  double dydx;
  if (S.y < 0)
  {
    dydx = -A * TwoPiLambda * cospi((2.0 * S.x) / lambda);
  }
  else
  {
    dydx = A * TwoPiLambda * cospi((2.0 * S.x) / lambda);
...
__device__ double sin_curve(double xs, double4 xa, double A, double lambda, double C)
{
  double ys;
  if (xa.y < 0.0)
  {
    ys = -A * sinpi((2.0 * xs) / lambda) - C;
  }
  else
  {
    ys = A * sinpi((2.0 * xs) / lambda) + C;
...
__device__ double dfunc_dx(double4 xa, double xs, double A, double lambda, double C)
{
  double ys = sin_curve(xs, xa, A, lambda, C);
  ...
  double dydx = dydx_sin_curve(S, A, lambda);
  double d2y_dx2 = d2y_dx2_sin_curve(S, A, lambda);

  double dfunc_dx_value = 1.0 + dydx * dydx + (ys - xa.y) * d2y_dx2;
  return dfunc_dx_value;
}

__device__ double func(double4 xa, double xs, double A, double lambda, double C)
{
  double ys = sin_curve(xs, xa, A, lambda, C);
  ...
  double func_value = -xa.x + xs + (ys - xa.y) * dydx;
  return func_value;
}

__device__ double3 normal_sine_curve(double3 S, double A, double lambda)
{
  double TwoPiLambda = ((2.0 * PI) / lambda);
  double param = A * TwoPiLambda * cospi((2.0 * S.x) / lambda);
  double denom = sqrt(param * param + 1.0);

  double3 normal_surf;
  normal_surf.x = param / denom;
  normal_surf.z = 0.0;
  if (S.y < 0.0)
  {
    normal_surf.y = 1.0 / denom;
  }
  else
  {
    normal_surf.y = -1.0 / denom;
...
__device__ double bisection(double xl, double xu, double4 xa, double A, double lambda, double C)
{
  double es = 0.5; //percent
  int imax = 20;

  double fl = func(xa, xl, A, lambda, C);
  double fu = func(xa, xu, A, lambda, C);
...
__device__ double3 PointOnSurface(double4 xa, double A, double lambda, double C)
{
  double xl1, xu1, xs1, xl2, xu2, xs2, chck1, chck2, xs, ys;
  double3 S;

  xl1 = xa.x - 0.25 * lambda;
  xu1 = xa.x;

  xl2 = xa.x;
  xu2 = xa.x + 0.25 * lambda;

  //bisection on each section:
  xs1 = bisection(xl1, xu1, xa, A, lambda, C);
  xs2 = bisection(xl2, xu2, xa, A, lambda, C);
  ...
  {
    chck1 = dfunc_dx(xa, xs1, A, lambda, C);
  }
  ...
  {
    chck2 = dfunc_dx(xa, xs2, A, lambda, C);
  }
...
__device__ double2 dxsys_dxa(double3 S, double A, double lambda, int i)
{
  double2 dxsysdxa;
  double dysdxs = dydx_sin_curve(S, A, lambda);
  if (i == 1)
  {
    dxsysdxa.x = 1.0;
...
__device__ double2 dN_dxs(double3 S, double A, double lambda)
{
  double2 dNdxs;
  ...
  double param = (A * TwoPiLambda) * cospi((2.0 * S.x) / lambda);
  double denom = param * param + 1.0;

  double numer1 = A * TwoPiLambda * TwoPiLambda * sinpi((2.0 * S.x) / lambda);
  dNdxs.x = - numer1 / (denom * sqrt(denom));

  double numer2 = A * A * TwoPiLambda * TwoPiLambda * TwoPiLambda
      * cospi((2.0 * S.x) / lambda) * sinpi((2.0 * S.x) / lambda);
  dNdxs.y = - numer2 / (denom * sqrt(denom));
...
__global__ void calculate_BodyWallInteraction(double3 *d_dUbdy_dxa,
  ...
  double sigma_bdy, double A, double lambda, double C, int N)
{
      ...
      double y = sin_curve(xa.x, xa, A, lambda, C);
      ...
      if (chk1 > chk2)
      {
        ...
        S = PointOnSurface(xa, A, lambda, C);
        Nhat = normal_sine_curve(S, A, lambda);
        ...
        r_alpha = la * abs(dot_na_Nhat) + da - dot_Nhat_xa_S;
          ...
          double2 dNdxs = dN_dxs(S, A, lambda);
          double2 dxys_dxa1 = dxsys_dxa(S, A, lambda, 1);
          double2 dxys_dxa2 = dxsys_dxa(S, A, lambda, 2);
          double dysdxs = dydx_sin_curve(S, A, lambda);
      ...
      else //bacteria is not in contact with surface
      {
        dUbdy_dxa.x = 0.0;
        dUbdy_dxa.y = 0.0;
        dUbdy_dxa.z = 0.0;
  ...
  curandState *state, float4 *d_random_numbers_noise)
{
    ...
    if (d_tumble_flag[gtid] == 1) //tumbling
    {
      ...
      else //tumble
      {
        ...
        //rotation matrix
        double R[2][2];
        R[0][0] = cospi(angle / 180.0); //convert to radians
        R[0][1] = -sinpi(angle / 180.0);
        R[1][0] = sinpi(angle / 180.0);
        R[1][1] = cospi(angle / 180.0);
    ...
    if (x_new.x < (-0.5 * L))
    {
      double delta_x = (-0.5 * L) - x_new.x;
      x_new.x = 0.5 * L - delta_x;
    }
    else if (x_new.x > (0.5 * L))
    {
      double delta_x = x_new.x - (0.5 * L);
      x_new.x = (-0.5 * L) + delta_x;
    }
...
void load_textfile_sim_parameters( char filename[],
  ...
  double& C, double& L, double& A, double& lambda,
...
void initial_loading(double4 x[], double4 n[], int N, double C, double L, double A,
  ...
  double delta_run, double delta_tumble, double avg_n_tumble, double std_n_tumble)
{
  ...
  string headers("sim_num, case_num, dt, time_save, start_time, final_time, N, l, d, C, L, A, lambda, epsilon_r, sigma_bdy, inverse_Pe_T, inverse_Pe_parallel, inverse_Pe_perp, inverse_Pe_R, delta_run, delta_tumble, avg_n_tumble, std_n_tumble");
...
int main(void)
{
    ...
    cout << endl<<"==============="<<endl
         ...
         << "Pe_T = " << Pe_T << endl
         << "Pe_parallel = " << Pe_parallel << endl
         << "Pe_perp = " << Pe_perp << endl
         << "Pe_R = " << Pe_R << endl
    ...
    print_to_file_input(sim_num, case_num, dt, time_save, start_time, final_time,
      N, l, d, C, L, A, lambda, epsilon_r, sigma_bdy,
      Pe_T, Pe_parallel, Pe_perp, Pe_R, delta_run, delta_tumble, avg_n_tumble, std_n_tumble);
  //-----End: simulation input-----
    ...
    initial_loading(x, n, N, C, L, A, l, d, t_run, t_tumble, delta_run, (final_time - start_time));
