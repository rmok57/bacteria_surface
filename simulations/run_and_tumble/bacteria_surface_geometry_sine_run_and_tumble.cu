...
//bacteria surface geometry, sine boundary, run and tumble
...
__global__ void time_marching(double4 *d_x, double4 *d_n,
  ...
  curandState *state, double *d_vMF_angle, int vMF_n)
{
	int gtid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (d_tumble_flag[gtid] == 1) //tumbling
    {
      ...
      else //tumble
      {
        ...
        angle = d_vMF_angle[indx];

        //rotation matrix
        double R[2][2];
        R[0][0] = cospi(angle / 180); //convert to radians
        R[0][1] = -sinpi(angle / 180);
        R[1][0] = sinpi(angle / 180);
        R[1][1] = cospi(angle / 180);
  ...
  double delta_run, double delta_tumble, int kappa, int vMF_n)
{
  ...
  string headers("sim_num, case_num, dt, time_save, start_time, final_time, N, l, d, C, L, A, lambda, epsilon_r, sigma_bdy, inverse_Pe_T, inverse_Pe_parallel, inverse_Pe_perp, inverse_Pe_R, delta_run, delta_tumble, kappa, vMF_n");
